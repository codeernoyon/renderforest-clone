...
const Header = () => {
  ...
  const [navBerSticky, setNavBerSticky] = useState(false);
  const { navs } = HeaderData;

  const navSticky = () => {
    window.scrollY > 20 ? setNavBerSticky(true) : setNavBerSticky(false);
  };
  useEffect(() => {
    // add event active listener (react-scroll)
  ...
  return (
    <header
      className={`px-7 fixed top-0 w-[100vw] z-50 ${
        navBerSticky &&
        "backdrop-filter backdrop-blur-lg bg-opacity-80 shadow-lg"
      }`}
    >
      <div className="flex items-center">
        ...
        <nav className="flex justify-between items-center py-3">
          {/* ---=== Nav ===--- */}
          <ul className=" space-x-10 px-16 md:mr-[25%] flex">
            {navs?.map((item, index) => (
          ...
          <div className=" hidden">
            {/* ----==== Toggle Icon ====---- */}
            <div
              className={`h-10 w-10 flex flex-col items-center justify-center cursor-pointer`}
              onClick={() => setToggle(!toggle)}
            >
              <div
                className={`h-1 w-9  bg-slate-400 rounded-full relative transition duration-300 ${
                  toggle ? "rotate-[45deg] top-2" : "rotate-0 bottom-0"
                }`}
              ></div>
              <div
                className={`h-1 w-9  bg-slate-400 mt-[7px] rounded-full transition duration-300 relative ${
                  toggle ? "-rotate-[45deg] bottom-[2px]" : "rotate-0"
                }`}
              ></div>
            </div>
            {/* ----==== Toggle Nav ====---- */}
            <div className="absolute hidden w-screen h-screen left-0 bg-slate-400">
              <ul className="w-[250px] hidden h-full relative -right-10 bg-slate-500">
                {navs.map((item, index) => (
                  <div
                    key={index.id}
                    className="flex items-center px-4 py-3 rounded-lg hover:bg-slate-300 transition duration-500"
                  >
                    <div
                      className={`flex justify-center items-center p-1 rounded-xl mr-5 ${item.topItem.backGround} `}
                    >
                      <span>{item.topItem.img}</span>
                    </div>
                    <span>{item.topItem.title}</span>
                  </div>
                ))}
              </ul>
            </div>
